...
public class FileStorageService implements IFileStorageService{
    ...
    @Override
    public Resource descargarArchivo(String tokenFile) {
        try {
            Path filePath = this.fileStorageLocation.resolve(tokenFile).normalize();
            Resource resource = new UrlResource(filePath.toUri());
            if(resource.exists()) {
                return resource;
            } else {
                throw new MyFileNotFoundException("File not found " + tokenFile);
            }
